...
| OpenAI retry loop | `OpenAIProvider.Complete` | exponential backoff (≤ 10 attempts, max 300s) | ctx cancel returns early |
...
- **Raw `net/http` for LLM calls, no SDK.** The OpenAI-compatible provider serializes messages and tools as `map[string]any`, consumes streaming chat-completion events directly, applies exponential-backoff retry on transient errors (HTTP > 403, EOF, `io.ErrUnexpectedEOF`, and closed connections; up to 10 attempts), and exposes `reasoning_content` and `extra_body` passthrough fields. Don't reintroduce `openai-go`.
...
- The OpenAI provider's retry loop is bounded by `maxAttempts = 10` with exponential backoff capped at 300s. It retries truncated streams (`io.EOF` / `io.ErrUnexpectedEOF`), closed connections, and HTTP statuses > 403. Adding new retryable conditions must remain conservative — we'd rather surface a 4xx error to the user than loop on a bad request.
...
| OpenAI provider | 10 | Exponential | 300s |

The OpenAI retry policy is bounded at 10 attempts with a 300-second backoff cap. The messaging channels should converge on a shared `retryableSend` helper with bounded attempts and exponential backoff.
