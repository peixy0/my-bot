...
| Feishu text/image/post processing | Feishu inbound | fire-and-forget per-message goroutine; post downloads at most four images sequentially, then enqueues one multi-image event | publishes through `inbox.Publish` with timeout |
...
- Feishu inbound's per-message processing goroutines (`processTextMessage`, `processImageMessage`, `processPostMessage`) and WeChat inbound's `processMessage` are fire-and-forget — every error path logs at `Warn` and either retries (WeChat QR status) or surfaces a single user-visible failure message via `outbound.Send`. `processPostMessage` downloads at most four images sequentially while it builds one `ImageInputEvent`, so a failed download can be represented by its marker in place without post-processing the assembled text. Don't add new fire-and-forget paths without the same discipline.
