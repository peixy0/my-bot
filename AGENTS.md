...
| `ConversationWorker.Run` (per chat) | chatSession | conversation state, `Control` chan, `Events` chan, `MessageInbox` chan, `abortCh` | none within worker |
...
| OpenAI retry loop | `OpenAIProvider.Complete` | exponential backoff (≤ 99 attempts, max 1800s) | ctx cancel returns early |
...
- **Per-arrival Outbound instances.** Each inbound message creates a fresh `Outbound` instance. An idle worker drains the current `MessageInbox` snapshot as one batch and uses the first event's `Outbound`; later instances in that batch are intentionally unused. The selected `Outbound`'s mutable streaming state (`partial`, `stream`) is owned exclusively by the worker goroutine — no concurrent access. In-loop messages continue using the `Outbound` that started the active agent loop. Feishu reactions do not rely on sender identity: `add_reaction` requires an explicit message ID exposed in the LLM input.
- **Per-run Registry rebuild.** `BuildRegistry(sender)` is called once for each idle message batch or background run. The outbound toolset (`send_image`/`send_file`/`add_reaction`) comes from the batch's first `Sender`, so the registry reflects only the active platform. There is never a conflict between same-named tools from different platforms. Registry construction remains cheap and happens at human interaction speed, not per token.
...
- **Live user input uses a separate message inbox, not the worker event queue.** An idle `ConversationWorker` blocks for the first ordinary text/image, then immediately drains the current `MessageInbox` snapshot and sends the ordered batch through one agent loop. Messages arriving after that drain remain pending unless `HumanInputOrchestrator` drains them after a tool dispatch and injects them in-loop. There is no debounce window and a completion without tools does not get interrupted. `/queue` wraps input in `QueuedInputEvent` and publishes it to `Events` for an explicitly later turn.
- **Worker control events are prioritized without interrupting active agents.** `NewSessionEvent`, config queries/changes, dump/resume, and reboot flow through `ConversationWorker.Control`; heartbeat, cron, compression, and queued user input remain on `Events` because they invoke an LLM. The worker drains control events before entering its ordinary select, but only reboot also signals `abortCh`.
- **Reboot is irreversible and best-effort once started.** `/reboot` synchronously sends each session a high-priority `RebootCommand`. Active agents are aborted; idle and aborted workers handle the command before ordinary inboxes, dump their conversation, report the result, and exit. Pending `MessageInbox` and `Events` content is abandoned. The Scheduler checkpoints successful dumps and returns `ErrReboot` even if a session dump or checkpoint write fails; failed conversations may be lost.
...
- **Raw `net/http` for LLM calls, no SDK.** The OpenAI-compatible provider serializes messages and tools as `map[string]any`, consumes streaming chat-completion events directly, applies exponential-backoff retry on transient errors (HTTP > 403, EOF, `io.ErrUnexpectedEOF`, and closed connections; up to 99 attempts), and exposes `reasoning_content` and `extra_body` passthrough fields. Don't reintroduce `openai-go`.
...
- `ConversationWorker.Control` is the worker-owned high-priority control inbox. `chatSession.publishEvent` is the sole classifier that routes non-LLM control events there. `chatSession.stopAndSnapshot` publishes `RebootCommand` before racing the result against `abortCh`: an idle worker consumes the command directly, while an active agent consumes the abort and the worker handles the already-pending command immediately after the agent returns. Reboot handling stops the heartbeat, dumps once, reports once, and exits without consuming ordinary inboxes.
- `ConversationWorker.cfg` is a per-worker `Config` clone; model and vision changes affect only that worker. The base `Agent` is shared and never forked — `http.Client` is thread-safe. **Shallow-copy caveat: `ForSession` does `merged := *c` (top-level value copy), so `merged.LLM` / `merged.Context` are independent value copies. However, any map fields within them (notably `LLM.ExtraBody` and `Presets`) are reference copies — they share the underlying map with the global config or the model preset entry. This is safe ONLY because no code mutates these maps after startup. If you ever write to `cfg.LLM.ExtraBody` or `cfg.Presets`, you MUST deep-copy first with `maps.Clone()` or equivalent. Model preset `ApplyTo` assigns `target.ExtraBody` by reference — same rule applies.**
...
- The OpenAI provider's retry loop is bounded by `maxAttempts = 99` with exponential backoff capped at 1800s. It retries truncated streams (`io.EOF` / `io.ErrUnexpectedEOF`), closed connections, and HTTP statuses > 403. Adding new retryable conditions must remain conservative — we'd rather surface a 4xx error to the user than loop on a bad request.
...
- `LLMConfig` exposes `top_k`, `context_window`, `vision`, `extra_body`, and the standard `temperature`/`top_p` for non-OpenAI providers. `Config.Presets` is a map of named presets (`Preset`) selectable by session or slash command.
- `ForSession(chatID)` value-copies the global config (top-level struct copy via `merged := *c`), applies the per-chat `sessions[chatID]` model and compression-model names, and permits either preset names or direct provider model names. Map fields (`Presets`, `LLM.ExtraBody`) are reference-copied — they share underlying data with the global config. This is safe because these maps are read-only after startup. Sessions never mutate the global config. See Invariants for details.
...
| OpenAI provider | 99 | Exponential | 1800s |

The OpenAI retry policy is bounded at 99 attempts with a 1800-second backoff cap. The messaging channels should converge on a shared `retryableSend` helper with bounded attempts and exponential backoff.
